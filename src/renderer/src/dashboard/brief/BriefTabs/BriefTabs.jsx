...
import BriefTable from './BriefTable/BriefTable';
import { Plus } from 'lucide-react';
import { Drawer } from '../../../components/common/Drawer';
import { BriefBuilder } from '../../builders/BriefBuilder/BriefBuilder';
...
const adminTabs = [
  ...
  { id: 'Recent Update', key: 'recentupdate' },
...
export default function BriefTabs() {
  ...
  } = useFetchData(`/briefs?filter=${filterName}&page=${pageNo}`);

  const briefs = briefResponse?.data || [];

  const { data: countResponse, refetch: refetchCounts } = useFetchData('/briefs/counts');
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-zinc-900">
      ...
      <div className="mt-6">
        <BriefTable tableData={briefs} loading={loading} error={error} />
      </div>
      ...
      {isDrawerOpen && (
        <Drawer open={isDrawerOpen} setOpen={setIsDrawerOpen} width="60%">
          <BriefBuilder onSuccess={handleBriefSuccess} />
        </Drawer>
      )}
    </div>
  );
