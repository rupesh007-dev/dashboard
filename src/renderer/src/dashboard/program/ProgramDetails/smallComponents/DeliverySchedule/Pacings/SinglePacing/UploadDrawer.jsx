...
export default function UploadDrawer({ pacingId }) {
  ...
  const uploadToApi = async (file) => {
    ...
    try {
      setIsUploading(true);
      const url = `${import.meta.env.VITE_BASE_URL}/bulk-upload/leads`;
      const response = await fetch(url, {
        method: 'POST',
        // headers: {
        //   Authorization: Bearer ${user.token},
  ...
  return (
    <>
      {/* <PageMeta title="Upload Leads" description="This is Upload page." />
      <PageBreadcrumb pageTitle="Upload Leads" /> */}
